(() => {
  const API_URL = "mini%20apps/missions/BeDastAvardim/tc_store.php";
        ...
        <tr data-index="${index}" data-level-id="${escapeHtml(level.id)}">
          ...
          <td>
            <div class="tct-action-wrap">
              ...
              ${isPot ? `<a class="btn ghost" href="mini%20apps/missions/BeDastAvardim/pot_draw.php?level_id=${encodeURIComponent(level.id)}" target="_blank" rel="noopener">Open Draw</a>` : ''}
              ${isPot ? `<a class="btn ghost" href="mini%20apps/missions/BeDastAvardim/pot_export.php?level_id=${encodeURIComponent(level.id)}">Export</a>` : ''}
    ...
    const openPotSettingsDialog = (index) => {
      ...
      overlay.addEventListener("click", async (event) => {
        ...
        if (nextSettings.locked) {
          try {
            const response = await fetch("mini%20apps/missions/BeDastAvardim/pot_api.php", {
              method: "POST",
              credentials: "same-origin",
              headers: {"Content-Type": "application/json"},
