...
function tcSecurityRequireDatabaseRuntime(): void
{
  $context = tcDatabaseRuntimeContextForPath(__DIR__ . '/Setting.json');
  if (!is_array($context)) {
    throw new RuntimeException('TaskClub requires its registered database instance.');
  }
  tcInstanceWriteData($context['pdo'], $context['code'], 'storage_mode', [
    'mode' => 'database_only',
    'version' => 1,
  ]);
}

tcSecurityRequireDatabaseRuntime();
