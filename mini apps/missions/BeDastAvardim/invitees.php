...
<script src="mini%20apps/missions/BeDastAvardim/vendor/xlsx/xlsx.full.min.js" defer></script>
<script>
(() => {
  ...
  const postRevealAction = async (action, payload = {}) => {
    const response = await fetch('mini%20apps/missions/BeDastAvardim/invitees_password_guard.php', {
      method: 'POST',
  ...
  editSaveBtn?.addEventListener('click', async () => {
    ...
    try {
      const response = await fetch('mini%20apps/missions/BeDastAvardim/invitees_update.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          csrf: csrfToken,
  ...
  uploadBtn?.addEventListener('click', async () => {
    ...
    try {
      ...
      const response = await fetch('mini%20apps/missions/BeDastAvardim/invitees_upload.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  anyPasswordSaveBtn?.addEventListener('click', async () => {
    ...
    try {
      const response = await fetch('mini%20apps/missions/BeDastAvardim/invitees_login_settings.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          csrf: csrfToken,
  ...
  addInviteeBtn?.addEventListener('click', async () => {
    ...
    try {
      const response = await fetch('mini%20apps/missions/BeDastAvardim/invitees_add.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          csrf: csrfToken,
