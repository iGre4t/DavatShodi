(() => {
  ...
  const TASKS_ENDPOINT = 'mini%20apps/missions/LahzehEyd/TCT.php';
  const LOGS_ENDPOINT = 'mini%20apps/missions/LahzehEyd/tc_logs.php';
  ...
  function buildTaskControlCardMarkup(task) {
    ...
    const quizSrc = `mini%20apps/missions/LahzehEyd/TCQ.php?task_id=${encodeURIComponent(task.id)}`;
  ...
  function setupInviteePrizeTotalsExport() {
    ...
    openButton.addEventListener('click', async () => {
      ...
      try {
        ...
        const response = await fetch('mini%20apps/missions/LahzehEyd/invitees_prize_totals_export.php', {
