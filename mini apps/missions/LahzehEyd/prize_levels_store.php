...
/**
 * Transactional storage for Task Club prize levels.
 *
 * The default write target is restricted to the level file beside this helper.
 * Tests may define TC_PRIZE_LEVELS_ALLOWED_DIRECTORY before loading this file.
 */

const TC_PRIZE_LEVELS_FILENAME = 'TC Prize Levels.json';

if (!defined('TC_PRIZE_LEVELS_ALLOWED_DIRECTORY')) {
  define('TC_PRIZE_LEVELS_ALLOWED_DIRECTORY', __DIR__);
}

function tcPrizeLevelsDefaultPath(): string
{
  return rtrim((string)TC_PRIZE_LEVELS_ALLOWED_DIRECTORY, "\\/")
    . DIRECTORY_SEPARATOR
    . TC_PRIZE_LEVELS_FILENAME;
}

function tcPrizeLevelsNormalizePathToken(string $path): string
{
  $normalized = str_replace('\\', '/', $path);
  return PHP_OS_FAMILY === 'Windows' ? strtolower($normalized) : $normalized;
}

function tcPrizeLevelsIsAllowedPath(string $path): bool
{
  $expectedName = TC_PRIZE_LEVELS_FILENAME;
  ...
  $allowedDirectory = realpath((string)TC_PRIZE_LEVELS_ALLOWED_DIRECTORY);
  $targetDirectory = realpath(dirname($path));
  if (!is_string($allowedDirectory) || !is_string($targetDirectory)) {
    return false;
  }
  return tcPrizeLevelsNormalizePathToken($allowedDirectory)
    === tcPrizeLevelsNormalizePathToken($targetDirectory);
}

function tcPrizeLevelsIsList(array $value): bool
{
...
function tcPrizeLevelsStableId(string $name, int $score, int $index, int $attempt = 0): string
{
  return 'lvl_' . substr(
...
function tcPrizeLevelsReadAliasedValue(array $record, array $keys, bool &$found)
{
  foreach ($keys as $key) {
...
function tcPrizeLevelsParsePositiveInt($value): ?int
{
  if (is_int($value)) {
...
function tcPrizeLevelsNormalizeTextValue($value, bool $required, ?string &$reason): ?string
{
...
function tcPrizeLevelsNormalizePotSettings($value, ?string &$reason): ?array
{
  if ($value === null) {
    $source = [];
 ...
 */
function tcPrizeLevelsNormalizeRecords(
  array $records,
  ?string &$reason = null,
  ...
  if (!tcPrizeLevelsIsList($records)) {
    $reason = 'not_a_list';
    return null;
  }
  ...
  foreach ($records as $index => $record) {
    ...
    $scoreFound = false;
    $scoreRaw = tcPrizeLevelsReadAliasedValue($record, ['score', 'levelScore', 'level_score'], $scoreFound);
    $score = $scoreFound ? tcPrizeLevelsParsePositiveInt($scoreRaw) : null;
    if ($score === null) {
    ...
    $typeFound = false;
    $typeRaw = tcPrizeLevelsReadAliasedValue($record, ['type', 'levelType', 'level_type'], $typeFound);
    ...
    $descriptionRaw = tcPrizeLevelsReadAliasedValue(
      $record,
      ['description', 'describe', 'infoText', 'info_text'],
      $descriptionFound
    );
    $description = tcPrizeLevelsNormalizeTextValue($descriptionFound ? $descriptionRaw : null, false, $reason);
    if ($description === null) {
      return null;
    }

    $buttonFound = false;
    $buttonRaw = tcPrizeLevelsReadAliasedValue($record, ['buttonText', 'button_text'], $buttonFound);
    $buttonText = tcPrizeLevelsNormalizeTextValue($buttonFound ? $buttonRaw : null, false, $reason);
    if ($buttonText === null) {
      return null;
    }

    $potFound = false;
    $potRaw = tcPrizeLevelsReadAliasedValue($record, ['potSettings', 'pot_settings'], $potFound);
    $potSettings = tcPrizeLevelsNormalizePotSettings($potFound ? $potRaw : null, $reason);
    if ($potSettings === null) {
      return null;
    }
...
function tcPrizeLevelsVersion(array $records): string
{
  $encoded = json_encode(
 ...
 */
function tcPrizeLevelsMergeCurrentFields(array $submittedRecords, array $currentRecords): array
{
  $currentById = [];
  ...
  foreach ($submittedRecords as $submitted) {
    ...
    $submittedPot = tcPrizeLevelsReadAliasedValue(
      $submitted,
...
function tcPrizeLevelsDecodeFile(string $path): array
{
  if (!tcDbIsFile($path)) {
    return [
      'ok' => true,
  ...
  if (!is_array($decoded) || json_last_error() !== JSON_ERROR_NONE || !tcPrizeLevelsIsList($decoded)) {
    return ['ok' => false, 'exists' => true, 'records' => [], 'migrationNeeded' => false, 'reason' => 'invalid_json'];
  }
  $reason = null;
  $migrationNeeded = false;
  $records = tcPrizeLevelsNormalizeRecords($decoded, $reason, $migrationNeeded);
...
function tcPrizeLevelsWriteAll($handle, string $payload): bool
{
  $offset = 0;
...
function tcPrizeLevelsPrepareFile(string $directory, array $records): ?string
{
  $encoded = json_encode(
    array_values($records),
  ...
  $ok = tcPrizeLevelsWriteAll($handle, $encoded . PHP_EOL);
  fclose($handle);
  if (!$ok) {
    @tcDbUnlink($temporaryPath);
    return null;
  }
  $decoded = tcPrizeLevelsDecodeFile($temporaryPath);
...
function tcPrizeLevelsReplacePrepared(string $preparedPath, string $targetPath): bool
{
  if (PHP_OS_FAMILY !== 'Windows') {
    return @tcDbRename($preparedPath, $targetPath);
  }
  $rollbackPath = $targetPath . '.rollback';
  @tcDbUnlink($rollbackPath);
  $hadOriginal = tcDbIsFile($targetPath);
  if ($hadOriginal && !@tcDbRename($targetPath, $rollbackPath)) {
    return false;
  }
  if (@tcDbRename($preparedPath, $targetPath)) {
    @tcDbUnlink($rollbackPath);
    return true;
  }
  if ($hadOriginal && tcDbIsFile($rollbackPath)) {
    @tcDbRename($rollbackPath, $targetPath);
  }
  return false;
}

function tcPrizeLevelsRefreshBackup(string $path): bool
{
  $live = tcPrizeLevelsDecodeFile($path);
  if (!($live['ok'] ?? false) || !($live['exists'] ?? false) || ($live['migrationNeeded'] ?? false)) {
    return false;
  }
  $temporaryPath = tcPrizeLevelsPrepareFile(dirname($path), (array)$live['records']);
  if (!is_string($temporaryPath)) {
    return false;
  }
  $backupPath = $path . '.bak';
  $ok = tcPrizeLevelsReplacePrepared($temporaryPath, $backupPath);
  if (tcDbIsFile($temporaryPath)) {
    @tcDbUnlink($temporaryPath);
  }
  return $ok;
}

function tcPrizeLevelsAtomicWrite(string $path, array $records, ?bool &$backupSynced = null): bool
{
  $backupSynced = false;
  $temporaryPath = tcPrizeLevelsPrepareFile(dirname($path), $records);
  if (!is_string($temporaryPath)) {
    return false;
  }
  $ok = tcPrizeLevelsReplacePrepared($temporaryPath, $path);
  if (tcDbIsFile($temporaryPath)) {
    @tcDbUnlink($temporaryPath);
  }
  if (!$ok) {
  ...
  $backupSynced = tcPrizeLevelsRefreshBackup($path);
  if (!$backupSynced) {
    error_log('Task Club prize-level backup refresh failed: ' . $path);
  }
  return true;
}

function tcPrizeLevelsCorruptPath(string $path): string
{
  try {
...
function tcPrizeLevelsRestoreFromFile(string $sourcePath, string $targetPath): bool
{
  $source = tcPrizeLevelsDecodeFile($sourcePath);
  if (!($source['ok'] ?? false) || !($source['exists'] ?? false)) {
    return false;
  }
  $temporaryPath = tcPrizeLevelsPrepareFile(dirname($targetPath), (array)$source['records']);
  if (!is_string($temporaryPath)) {
    return false;
  }
  $corruptPath = '';
  if (tcDbIsFile($targetPath)) {
    $corruptPath = tcPrizeLevelsCorruptPath($targetPath);
    if (!@tcDbRename($targetPath, $corruptPath)) {
      @tcDbUnlink($temporaryPath);
      return false;
    }
  }
  if (@tcDbRename($temporaryPath, $targetPath)) {
    return true;
  }
  @tcDbUnlink($temporaryPath);
  if ($corruptPath !== '' && tcDbIsFile($corruptPath)) {
    @tcDbRename($corruptPath, $targetPath);
  }
  return false;
}

function tcPrizeLevelsRecoverInterruptedReplace(string $path): void
{
  $rollbackPath = $path . '.rollback';
  if (!tcDbIsFile($rollbackPath)) {
    return;
  }
  $rollback = tcPrizeLevelsDecodeFile($rollbackPath);
  if (!tcDbIsFile($path)) {
  ...
  if (($rollback['ok'] ?? false) && ($rollback['exists'] ?? false)) {
    $corruptPath = tcPrizeLevelsCorruptPath($path);
    if (@tcDbRename($path, $corruptPath)) {
      if (!@tcDbRename($rollbackPath, $path)) {
        @tcDbRename($corruptPath, $path);
      }
    }
  }
}

function tcPrizeLevelsLoadUnderLock(string $path): array
{
  tcPrizeLevelsRecoverInterruptedReplace($path);
  $live = tcPrizeLevelsDecodeFile($path);
  if (($live['ok'] ?? false) && ($live['exists'] ?? false)) {
  ...
  $backupPath = $path . '.bak';
  if (tcDbIsFile($backupPath)) {
    $backup = tcPrizeLevelsDecodeFile($backupPath);
    if (($backup['ok'] ?? false) && ($backup['exists'] ?? false)
      && tcPrizeLevelsRestoreFromFile($backupPath, $path)) {
      $restored = tcPrizeLevelsDecodeFile($path);
...
function tcPrizeLevelsWithExclusiveLock(string $path, callable $operation): array
{
  if (!tcPrizeLevelsIsAllowedPath($path)) {
    return ['ok' => false, 'reason' => 'path_not_allowed', 'records' => [], 'version' => ''];
  }
  $lock = tcDbFopen($path . '.lock', 'c+b');
  if ($lock === false) {
  ...
  try {
    ...
    error_log('Task Club prize-level store failure: ' . $error->getMessage());
 ...
 */
function tcPrizeLevelsReadSnapshot(string $path = ''): array
{
  $targetPath = $path !== '' ? $path : tcPrizeLevelsDefaultPath();
  return tcPrizeLevelsWithExclusiveLock($targetPath, static function () use ($targetPath): array {
    $loaded = tcPrizeLevelsLoadUnderLock($targetPath);
    if (!($loaded['ok'] ?? false)) {
      return [
        'ok' => false,
    ...
    if (!empty($loaded['migrationNeeded'])) {
      if (!tcPrizeLevelsAtomicWrite($targetPath, $records, $backupSynced)) {
        return ['ok' => false, 'reason' => 'migration_write_failed', 'records' => [], 'version' => ''];
      }
      $migrated = true;
    } elseif (!empty($loaded['exists'])) {
      $backup = tcPrizeLevelsDecodeFile($targetPath . '.bak');
      $backupMatchesLive = ($backup['ok'] ?? false)
        && ($backup['exists'] ?? false)
        && empty($backup['migrationNeeded'])
        && hash_equals(
          tcPrizeLevelsVersion($records),
          tcPrizeLevelsVersion((array)($backup['records'] ?? []))
        );
      if (!$backupMatchesLive) {
        $backupSynced = tcPrizeLevelsRefreshBackup($targetPath);
      }
    }
    ...
    return [
      ...
      'reason' => '',
      'records' => $records,
      'version' => tcPrizeLevelsVersion($records),
      'migrated' => $migrated,
 ...
 */
function tcPrizeLevelsReplaceIfVersion(
  string $path,
  array $records,
  string $expectedVersion
): array {
  return tcPrizeLevelsWithExclusiveLock($path, static function () use ($path, $records, $expectedVersion): array {
    $loaded = tcPrizeLevelsLoadUnderLock($path);
    if (!($loaded['ok'] ?? false)) {
    ...
    if (!empty($loaded['migrationNeeded'])) {
      ...
      if (!tcPrizeLevelsAtomicWrite($path, $current, $migrationBackupSynced)) {
        return ['ok' => false, 'reason' => 'migration_write_failed', 'records' => [], 'version' => ''];
      }
    }
    $currentVersion = tcPrizeLevelsVersion($current);
    ...
    $recordsWithPreservedFields = tcPrizeLevelsMergeCurrentFields($records, $current);
    $reason = null;
    $migrationNeeded = false;
    $normalized = tcPrizeLevelsNormalizeRecords($recordsWithPreservedFields, $reason, $migrationNeeded);
    ...
    $backupSynced = false;
    if (!tcPrizeLevelsAtomicWrite($path, $normalized, $backupSynced)) {
      return [
        'ok' => false,
        'reason' => 'write_failed',
        'records' => $current,
    ...
    return [
      ...
      'reason' => '',
      'records' => $normalized,
      'version' => tcPrizeLevelsVersion($normalized),
      'backupSynced' => $backupSynced
    ];
  });
}
