...
function tcStartIsolatedSession(): void
{
  ...
  session_name('TASKCLUB' . substr(hash('sha256', __DIR__), 0, 12));
...
{
  $levels = [];
  foreach (readPrizeLevelRecords($path) as $item) {
    if (!is_array($item)) {
      continue;
    }
...
function parseRewardStoredList(string $raw, ?bool &$valid = null, ?bool &$wasJson = null): array
{
  $valid = true;
  $wasJson = false;
  $trimmed = trim($raw);
  ...
  if (str_starts_with($trimmed, '[')) {
    ...
    if (!is_array($decoded) || (function_exists('array_is_list') && !array_is_list($decoded))) {
      $valid = false;
      return [];
    }
    $result = [];
...
function serializeRewardStoredList(array $items): string
{
  $clean = [];
  foreach ($items as $item) {
  ...
  $encoded = json_encode(array_values($clean), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_INVALID_UTF8_SUBSTITUTE);
  return is_string($encoded) ? $encoded : '[]';
}

function parseRewardPrizeNames(string $raw, ?bool &$valid = null, ?bool &$wasJson = null): array
{
  $valid = true;
  $wasJson = false;
  $trimmed = trim($raw);
  if ($trimmed === '') return [];
  if (str_starts_with($trimmed, '[')) {
    return parseRewardStoredList($trimmed, $valid, $wasJson);
  }
  $legacyEntries = parseCommaSeparatedList($trimmed);
  $names = [];
  foreach ($legacyEntries as $entry) {
    $separatorIndex = strpos($entry, ':');
    $name = $separatorIndex !== false && $separatorIndex > 0
      ? trim((string)substr($entry, $separatorIndex + 1))
      : trim((string)$entry);
    if ($name !== '') $names[] = $name;
  }
  return $names;
...
function tcRewardPrizeIndex(array $prizes, string $prizeId, string $inventoryName = ''): int
{
  foreach ($prizes as $index => $item) {
    ...
    if ($prizeId !== '' && (string)($item['id'] ?? '') === $prizeId) return (int)$index;
  }
  if ($inventoryName !== '') {
    foreach ($prizes as $index => $item) {
      if (is_array($item) && trim((string)($item['name'] ?? '')) === $inventoryName) return (int)$index;
    }
  }
  return -1;
}
...
): ?array
{
  ...
  foreach (array_reverse($entries) as $entry) {
    ...
    if ($levelId !== '' && (string)($storedLevel['id'] ?? '') !== $levelId) {
      return ['requestConflict' => true, 'conflictReason' => 'level_id'];
    }
    if ($cardIndex !== null && (int)($entry['cardIndex'] ?? -1) !== $cardIndex) {
      return ['requestConflict' => true, 'conflictReason' => 'card_index'];
    }
    return $entry;
  }
  return null;
...
function tcRewardWeightedPrizeIndex(array $prizes, ?int $ticket = null): int
{
  ...
  $total = 0;
  foreach ($prizes as $index => $item) {
    if (!is_array($item) || (bool)($item['isFake'] ?? false)) continue;
    $name = trim((string)($item['name'] ?? ''));
    $displayName = trim((string)($item['onWheelName'] ?? $name));
    $last = max(0, (int)($item['last'] ?? 0));
    if (($name === '' && $displayName === '') || $last <= 0) continue;
    if ($last > PHP_INT_MAX - $total) return -1;
    $weights[(int)$index] = $last;
    $total += $last;
  }
  if ($total <= 0) return -1;
  $selectionTicket = $ticket ?? random_int(1, $total);
  if ($selectionTicket < 1 || $selectionTicket > $total) return -1;
  foreach ($weights as $index => $weight) {
    $selectionTicket -= $weight;
    if ($selectionTicket <= 0) return (int)$index;
  }
  return -1;
}

/**
 * Resolves award requests interrupted between inventory and CSV commits.
...
function tcRewardReconcileAwardTransactions(
  ...
  foreach ($entries as $entry) {
    if (is_array($entry)) {
      $ledgerAwardId = trim((string)($entry['awardId'] ?? ''));
      if ($ledgerAwardId !== '') $ledgerAwardIds[$ledgerAwardId] = true;
    }
  }
  ...
  foreach ($entries as $entry) {
    ...
    if ($status === 'awarded') {
      if ($markerPresent) {
        array_splice($markers, (int)$markerPosition, 1);
        $prizes[$prizeIndex]['pendingAwardIds'] = $markers;
    ...
    $workId = trim((string)($entry['workId'] ?? ''));
    ...
    $csvWon = false;
    if ($rowIndex >= 1 && $wonLevelIdsIndex >= 0 && $levelId !== '') {
      $storedLevelIdsValid = true;
      $storedLevelIds = parseRewardStoredList(
        (string)($rows[$rowIndex][$wonLevelIdsIndex] ?? ''),
        $storedLevelIdsValid
      );
    ...
    if ($csvWon) {
      ...
      $logUpdates[] = [$awardId, [
        'status' => 'awarded',
        'awardedAt' => (string)($entry['awardedAt'] ?? '') ?: gmdate('c'),
    ...
    if ($markerPresent) {
      ...
      $last = max(0, (int)($prizes[$prizeIndex]['last'] ?? 0));
      $prizes[$prizeIndex]['last'] = min($quantity, $last + 1);
      array_splice($markers, (int)$markerPosition, 1);
      $prizes[$prizeIndex]['pendingAwardIds'] = $markers;
      $inventoryChanged = true;
    }
    $logUpdates[] = [$awardId, [
      ...
      'cancellationReason' => ($markerPresent || $restorePending)
        ? 'recovered_before_csv_commit'
        : 'recovered_before_inventory_commit',
      'inventoryRestored' => $markerPresent || $restorePending,
      'reconciledAt' => gmdate('c')
    ]];
  }
...
function readTaskQuizAttemptState(string $workId, string $taskId): ?array
{
  ...
  $rawStartedAt = max(0, (int)($state['startedAt'] ?? 0));
  if (array_key_exists('quizStarted', $state) || array_key_exists('quiz_started', $state)) {
    $quizStarted = normalizeTaskBoolValue($state['quizStarted'] ?? ($state['quiz_started'] ?? false));
  } else {
    $quizStarted = $rawStartedAt > 0 && $answeredCount > 0;
  }
  $startedAt = $quizStarted ? $rawStartedAt : 0;
...
function writeTaskQuizAttemptState(string $workId, string $taskId, array $state): void
{
  ...
  $startedAt = array_key_exists('startedAt', $state)
    ...
    : time();
  $quizStarted = array_key_exists('quizStarted', $state) || array_key_exists('quiz_started', $state)
    ? normalizeTaskBoolValue($state['quizStarted'] ?? ($state['quiz_started'] ?? false))
    : $startedAt > 0;
  if (!$quizStarted) {
    $startedAt = 0;
  }
...
function markTaskQuizAttemptStarted(string $workId, string $taskId): ?array
{
  $state = readTaskQuizAttemptState($workId, $taskId);
  if (!is_array($state) || !($state['questionCodes'] ?? [])) {
    return null;
  }
  if (!empty($state['quizStarted']) && (int)($state['startedAt'] ?? 0) > 0) {
    return $state;
  }
  $state['quizStarted'] = true;
  $state['startedAt'] = time();
  writeTaskQuizAttemptState($workId, $taskId, $state);
  $refreshed = readTaskQuizAttemptState($workId, $taskId);
  return is_array($refreshed) ? $refreshed : $state;
}

function startTaskQuizAttempt(string $workId, string $taskId, array $questions, array $settings, bool $usesCorrectAnswersToScore = true, ?int $startedAt = null): array
{
  ...
  $attemptStartedAt = $startedAt ?? time();
  writeTaskQuizAttemptState($workId, $taskId, [
    'questionCodes' => $questionCodes,
    'requiredCorrectAnswers' => $requiredCorrectAnswers,
    'answered' => [],
    'startedAt' => $attemptStartedAt,
    'quizStarted' => $attemptStartedAt > 0
  ]);
...
function resolveTaskQuizQuestionAwardScore(array $question, string $taskStatus): int
{
  if ($taskStatus === 'ended') {
    return normalizeTaskScoreValue($question['goldenTimeEndedScore'] ?? ($question['afterEndtimeScore'] ?? ($question['after_endtime_score'] ?? 0)));
  }
  return normalizeTaskScoreValue($question['activeDurationScore'] ?? ($question['active_duration_score'] ?? ($question['score'] ?? 0)));
}

function calculateTaskQuizQuestionScoreSum(array $attemptState, array $questionLookup, string $fallbackTaskStatus): int
{
  $answered = is_array($attemptState['answered'] ?? null) ? $attemptState['answered'] : [];
  $total = 0;
  foreach ($answered as $questionCode => $entry) {
    if (!is_array($entry) || empty($entry['correct'])) {
      continue;
    }
    $hasStoredScore = array_key_exists('awardedScore', $entry) || array_key_exists('awarded_score', $entry);
    $storedScore = max(0, normalizeTaskScoreValue($entry['awardedScore'] ?? ($entry['awarded_score'] ?? 0)));
    if ($hasStoredScore) {
      $total += $storedScore;
      continue;
    }
    $questionKey = strtoupper(trim((string)$questionCode));
    if ($questionKey === '' || !isset($questionLookup[$questionKey]) || !is_array($questionLookup[$questionKey])) {
      continue;
    }
    $total += resolveTaskQuizQuestionAwardScore($questionLookup[$questionKey], $fallbackTaskStatus);
  }
  return max(0, $total);
}
...
function recordTaskQuizAttemptAnswer(array $attemptState, string $questionCode, string $answer, array $questionLookup, string $taskStatus, bool $includeQuestionScore = true, ?int $fixedCorrectAnswerScore = null): array
{
  ...
  return [
    ...
    'correctCount' => $correctCount,
    'questionScoreAwarded' => $questionScoreAwarded
  ];
}
...
function isReturnChanceZeroCompletion(array $task, string $taskId, array $taskScoreMap, bool $isCompleted): bool
{
  ...
  if (!normalizeTaskBoolValue($task['anotherChanceIfZero'] ?? false)) {
    return false;
  }
  if (!array_key_exists($taskId, $taskScoreMap)) {
    return false;
  }
  return max(0, (int)($taskScoreMap[$taskId] ?? 0)) <= 0;
}

function removeCompletedTaskId(array $ids, string $taskId): array
{
  $filtered = [];
  foreach ($ids as $id) {
...
function completeConditionalQuizAttempt(
  ...
  $isAlreadyCompleted = in_array($taskId, $completedTaskIds, true);
  if (isReturnChanceZeroCompletion($task, $taskId, $taskScoreMap, $isAlreadyCompleted)) {
    $completedTaskIds = removeCompletedTaskId($completedTaskIds, $taskId);
  ...
  if ($awardedScore <= 0 && normalizeTaskBoolValue($task['anotherChanceIfZero'] ?? false)) {
    clearTaskQuizAttemptState($normalizedWorkId, $taskId);
    return [
      'ok' => true,
      'status' => 'ok',
...
function readTaskUserProgress(array $task, string $inviteesPath, string $inviteesMapPath, string $workId): array
{
  ...
  if (isReturnChanceZeroCompletion($task, $taskId, $taskScoreMap, $isCompleted)) {
    $isCompleted = false;
    $taskScore = 0;
  }
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
  ...
  // and bypassed the transactional card-award service. They are intentionally
  // retired; reward_flip is the only user-facing value-prize write path.
  if (in_array($action, ['log_prize', 'decrement_prize'], true)) {
    http_response_code(410);
    echo json_encode([
      'status' => 'error',
      'message' => 'این روش قدیمی ثبت جایزه غیرفعال شده است؛ لطفا صفحه را به‌روزرسانی کنید.'
    ], JSON_UNESCAPED_UNICODE);
    exit;
  }
  ...
  if ($action === 'task_fetch') {
    ...
    if ($taskType === 'conditional_quiz' && empty($progress['completed'])) {
      ...
      $quizStarted = is_array($attemptState) && !empty($attemptState['quizStarted']);
  ...
  if ($action === 'task_start_quiz') {
    ...
    if ($taskType === 'conditional_quiz') {
      $attemptState = markTaskQuizAttemptStarted($sessionWorkId, $taskId);
      if (!is_array($attemptState)) {
        echo json_encode(['status' => 'error', 'message' => 'شروع ماموریت ناموفق بود. دوباره وارد ماموریت شوید.']);
        exit;
      }
    }
    ...
    echo json_encode([
      'status' => 'ok',
      'startedAt' => max(0, (int)($attemptState['startedAt'] ?? 0)),
      'quizStarted' => !empty($attemptState['quizStarted'])
    ], JSON_UNESCAPED_UNICODE);
    exit;
  }
  ...
  if ($action === 'task_log_answer') {
    ...
    if ($taskType === 'conditional_quiz') {
      $startedAttemptState = markTaskQuizAttemptStarted($sessionWorkId, $taskId);
      if (is_array($startedAttemptState)) {
        $attemptState = $startedAttemptState;
      }
    }
    if ($taskType === 'conditional_quiz') {
  ...
  if ($action === 'reward_state') {
    ...
    $wonPrizeNamesOrdered = $wonPrizeEntries;
    $wonPrizeByLevelId = [];
    ...
    echo json_encode([
      ...
      'data' => [
        ...
        'wonPrizeNames' => $wonPrizeNamesOrdered,
  ...
  if ($action === 'reward_flip') {
    ...
    if ($requestId === '') {
      $requestId = 'legacy-' . bin2hex(random_bytes(16));
    }
    if (!preg_match('/^[A-Za-z0-9._:-]{8,128}$/', $requestId)) {
      echo json_encode(['status' => 'error', 'message' => 'شناسه درخواست جایزه معتبر نیست.']);
      exit;
    }
    $cardIndexRaw = $payload['cardIndex'] ?? null;
    $cardIndex = is_numeric($cardIndexRaw) ? (int)$cardIndexRaw : -1;
    if ($cardIndex < 0) {
      echo json_encode(['status' => 'error', 'message' => 'کارت انتخاب‌شده معتبر نیست.']);
      exit;
    }
    ...
    if (!tcRewardReconcileAwardTransactions(
      ...
      $releaseAwardLocks();
      echo json_encode(['status' => 'error', 'message' => 'بازیابی تراکنش قبلی جایزه ناموفق بود؛ هیچ جایزه جدیدی ثبت نشد.']);
      exit;
    }
    ...
    if (is_array($existingRequest)) {
      if (!empty($existingRequest['requestConflict'])) {
        $releaseAwardLocks();
        echo json_encode(['status' => 'error', 'message' => 'شناسه درخواست قبلا استفاده شده است.']);
        exit;
      }
      $existingStatus = (string)($existingRequest['status'] ?? '');
      if ($existingStatus === 'awarded') {
        $existingPrize = is_array($existingRequest['prize'] ?? null) ? $existingRequest['prize'] : [];
        $existingLevel = is_array($existingRequest['level'] ?? null) ? $existingRequest['level'] : [];
        $releaseAwardLocks();
        echo json_encode([
          'status' => 'ok',
          'data' => [
            'awardId' => (string)($existingRequest['awardId'] ?? ''),
            'requestId' => $requestId,
            'idempotentReplay' => true,
            'levelId' => (string)($existingLevel['id'] ?? ''),
            'levelName' => (string)($existingLevel['name'] ?? ''),
            'prizeName' => (string)($existingPrize['name'] ?? ''),
            'prizeValue' => (float)($existingPrize['value'] ?? 0),
            'cardIndex' => (int)($existingRequest['cardIndex'] ?? $cardIndex),
            'cardFlipsCount' => (int)($existingRequest['cardFlipsAfter'] ?? 0),
            'totalPrizeWon' => (float)($existingRequest['totalPrizeWonAfter'] ?? 0)
          ]
        ], JSON_UNESCAPED_UNICODE);
        exit;
      }
      $releaseAwardLocks();
      echo json_encode(['status' => 'error', 'message' => 'درخواست قبلی جایزه تکمیل نشد؛ وضعیت به‌روزرسانی شد، دوباره کارت را انتخاب کنید.']);
      exit;
    }

    $userScore = $scoreIndex >= 0 ? max(0, (int)($rows[$rowIndex][$scoreIndex] ?? 0)) : 0;
    $wonLevelHistoryValid = true;
    $wonLevelIds = parseRewardStoredList(
      (string)($rows[$rowIndex][$wonLevelIdsIndex] ?? ''),
      $wonLevelHistoryValid
    );
    if (!$wonLevelHistoryValid || count(array_unique($wonLevelIds)) !== count($wonLevelIds)) {
      $releaseAwardLocks();
      echo json_encode(['status' => 'error', 'message' => 'تاریخچه سطح‌های جایزه آسیب دیده است؛ جایزه‌ای ثبت نشد.']);
      exit;
    }
    $wonSet = [];
    foreach ($wonLevelIds as $token) {
    ...
    $levels = readPrizeLevelRecords($prizeLevelsPath);
    $valueLevelCount = count(array_filter($levels, static fn(array $level): bool => (string)($level['type'] ?? '') === 'value_sum'));
    $cardCount = max(9, $valueLevelCount);
    if ($cardIndex >= $cardCount) {
      $releaseAwardLocks();
      echo json_encode(['status' => 'error', 'message' => 'شماره کارت با تنظیمات فعلی سازگار نیست.']);
      exit;
    }
    $targetLevel = null;
    foreach ($levels as $level) {
    ...
    if ($selectedStoreIndex < 0 || !isset($prizes[$selectedStoreIndex])) {
      $releaseAwardLocks();
      echo json_encode(['status' => 'error', 'message' => 'انتخاب امن جایزه ناموفق بود؛ هیچ تغییری انجام نشد.']);
      exit;
    }
    $selectedPrize = $prizes[$selectedStoreIndex];
    $selectedPrizeId = trim((string)($selectedPrize['id'] ?? ''));
    ...
    $wonPrizeHistoryValid = true;
    $wonPrizeEntries = parseRewardPrizeNames(
      (string)($rows[$rowIndex][$wonPrizeIndex] ?? ''),
      $wonPrizeHistoryValid
    );
    if (!$wonPrizeHistoryValid || count($wonPrizeEntries) !== count($wonLevelIds)) {
      $releaseAwardLocks();
    ...
    $userFullName = resolveInviteeFullName(
      ...
      is_array($table['mapping'] ?? null) ? $table['mapping'] : [],
    ...
    $awardLogEntry = [
      ...
      'level' => [
        'id' => $targetLevelId,
        'name' => trim((string)($targetLevel['name'] ?? 'Level')),
        'requiredScore' => max(0, (int)($targetLevel['score'] ?? 0))
      ],
      'prize' => [
        'id' => $selectedPrizeId,
        'name' => $selectedPrizeName,
        'inventoryName' => $selectedPrizeNameRaw,
        'value' => $selectedPrizeValue,
        'inventoryBefore' => $selectedLast,
        'inventoryAfter' => max(0, $selectedLast - 1)
      ],
      'cardFlipsBefore' => $currentFlipCount,
      'cardFlipsAfter' => $currentFlipCount + 1,
    ...
    $wonPrizeEntries[] = $selectedPrizeName;
    $rows[$rowIndex][$wonPrizeIndex] = serializeRewardStoredList($wonPrizeEntries);
    ...
    if (!writeInviteesCsv($inviteesFilePath, $rows, false)) {
      $restorePrepared = tcPrizeAwardLogUpdate($prizeAwardLogPath, $awardId, [
        ...
        'inventoryRestoreStartedAt' => gmdate('c'),
        'inventoryRestored' => false,
        'lastError' => 'invitees_csv_write_failed'
      ]);
      if (!$restorePrepared) {
        // Keep the committed decrement and marker together. A later recovery
        // can safely decide what happened; restoring without this ledger phase
        // would make an update failure ambiguous and risk a double restore.
        $releaseAwardLocks();
        echo json_encode(['status' => 'error', 'message' => 'ذخیره نتیجه جایزه ناموفق بود؛ بازیابی امن در انتظار ثبت گزارش است.']);
        exit;
      }
      $prizes[$selectedStoreIndex]['last'] = min(
        max(0, (int)($prizes[$selectedStoreIndex]['quantity'] ?? 0)),
    ...
    foreach ($wonLevelIds as $idx => $levelIdToken) {
      ...
      $prizeName = trim((string)($wonPrizeEntries[$idx] ?? ''));
    ...
    echo json_encode([
      ...
      'data' => [
        ...
        'wonPrizeNames' => array_values($wonPrizeEntries),
        'levels' => $levelPayload
      ]
    ], JSON_UNESCAPED_UNICODE);
    exit;
  ...
  <head>
    ...
    <style nonce="<?= htmlspecialchars($cspNonce, ENT_QUOTES, 'UTF-8') ?>">
      ...
      * {
      ...
      .login-btn.quiz-nav-next:disabled {
        opacity: 0.6;
        cursor: not-allowed;
      }
      ...
      .quiz-area--manual-nav .quiz-timer-track {
        opacity: 0;
        pointer-events: none;
      }
  ...
  <body class="page-loading <?= $sessionPayload['authed'] ? 'authed' : 'no-auth' ?>">
    ...
    <script nonce="<?= htmlspecialchars($cspNonce, ENT_QUOTES, 'UTF-8') ?>">
      ...
      if (sessionInfo?.authed) {
        ...
        let rewardCardsStorageLoaded = false;
        let currentTaskId = '';
        ...
        const describeStatus = (settings) => {
          ...
          if (startRelation === 0 || endRelation === 0) {
            ...
            if (startRelation === 0 && startSeconds !== null && nowSeconds < startSeconds) return 'upcoming';
            if (endRelation === 0 && endSeconds !== null && nowSeconds >= endSeconds) return 'ended';
          }
          return 'active';
        };
        ...
        const hideConditionalQuizNextButton = () => {
          conditionalQuizPendingNext = null;
          if (quizAreaEl) {
          ...
          if (quizNextBtnEl instanceof HTMLButtonElement) {
            ...
            quizNextBtnEl.textContent = 'بعدی';
          }
        };

        const showConditionalQuizNextButton = (item, answerText, answerPayload) => {
          conditionalQuizPendingNext = { item, answerText, answerPayload };
          if (quizAreaEl) {
            quizAreaEl.classList.add('quiz-area--manual-nav');
          }
          if (quizFooterEl) {
            quizFooterEl.classList.remove('quiz-hidden');
          }
          if (quizNextBtnEl instanceof HTMLButtonElement) {
            quizNextBtnEl.textContent = 'بعدی';
        ...
        const startCurrentQuizTaskView = async () => {
          ...
          try {
            ...
            if (currentTaskType === 'conditional_quiz') {
              const startPayload = await postJson({ action: 'task_start_quiz', taskId: currentTaskId });
              if (startPayload?.alreadyCompleted) {
                throw new Error('این ماموریت قبلا انجام شده و امتیاز گرفته است.');
              }
            }
            quizLocked = false;
            openQuizOverlay();
        ...
        const getRewardCardCount = () => {
          ...
          const valueLevelCount = levels.filter((level) => String(level?.type || '') === 'value_sum').length;
          return Math.max(MIN_REWARD_CARD_COUNT, valueLevelCount);
        };

        const isValidRewardCardIndex = (index) => (
          Number.isInteger(index) && index >= 0 && index < getRewardCardCount()
        );

        const pruneLockedRewardCards = () => {
          let changed = false;
          rewardCardsLockedIndexes.forEach((index) => {
            if (!isValidRewardCardIndex(index)) {
              rewardCardsLockedIndexes.delete(index);
              changed = true;
            }
          });
          rewardCardsLockedPrizes.forEach((_prizeName, index) => {
            if (!isValidRewardCardIndex(index) || !rewardCardsLockedIndexes.has(index)) {
              rewardCardsLockedPrizes.delete(index);
              changed = true;
            }
          });
          return changed;
        ...
        const saveLockedRewardCards = () => {
          try {
            pruneLockedRewardCards();
            const indexes = Array.from(rewardCardsLockedIndexes).filter(isValidRewardCardIndex);
            localStorage.setItem(rewardCardLockStorageKey, JSON.stringify(indexes));
            const prizeEntries = [];
            rewardCardsLockedPrizes.forEach((prizeName, index) => {
              if (!isValidRewardCardIndex(index) || !rewardCardsLockedIndexes.has(index)) return;
          ...
          try {
            ...
            if (Array.isArray(indexes)) {
              for (const item of indexes) {
                const parsed = Number.isInteger(item) ? item : Number.NaN;
                if (isValidRewardCardIndex(parsed)) {
                  rewardCardsLockedIndexes.add(parsed);
                  if (rewardCardsLockedIndexes.size >= cardCount) break;
                }
              }
            }
          } catch {}
          try {
            ...
            if (Array.isArray(entries)) {
              for (const item of entries) {
                ...
                const parsedIndex = Number.isInteger(item.index) ? item.index : Number.NaN;
                const prizeName = String(item.prizeName || '').trim();
                if (isValidRewardCardIndex(parsedIndex) && rewardCardsLockedIndexes.has(parsedIndex) && prizeName !== '') {
                  rewardCardsLockedPrizes.set(parsedIndex, prizeName);
                }
              }
            }
          } catch {}
          rewardCardsStorageLoaded = true;
        };
        ...
        const getRewardLevelById = (levelId) => {
          const normalizedLevelId = String(levelId || '').trim();
          if (!normalizedLevelId) return null;
          const levels = Array.isArray(rewardsState?.levels) ? rewardsState.levels : [];
          return levels.find((level) => String(level?.id || '').trim() === normalizedLevelId) || null;
        };

        const getSelectedRewardLevel = () => getRewardLevelById(selectedRewardLevelId);
        ...
        const getCardsHintState = () => {
          ...
          if (rewardsCardsViewOpen && selectedRewardLevelId) {
            const selectedLevel = getSelectedRewardLevel();
            if (!selectedLevel || String(selectedLevel?.type || '') !== 'value_sum') {
              return { message: 'سطح جایزه انتخاب‌شده معتبر نیست', disableUnchosen: true };
            }
            if (Boolean(selectedLevel?.won)) {
              return { message: 'جایزه این سطح قبلا دریافت شده است', disableUnchosen: true };
            }
          }
          ...
          if (rewardsCardsViewOpen && selectedRewardLevelId && !Boolean(getSelectedRewardLevel()?.canFlip)) {
            return { message: 'این سطح هنوز آماده انتخاب کارت نیست', disableUnchosen: true };
          }
        ...
        const getWonPrizeNames = () => {
          if (Array.isArray(rewardsState?.wonPrizeNames)) {
            return rewardsState.wonPrizeNames
              .map((prizeName) => String(prizeName || '').trim())
              .filter((prizeName) => prizeName !== '');
          }
        ...
        const renderRewardRoadmap = () => {
          ...
          rewardRoadmapEl.innerHTML = levels.map((level) => {
            ...
            if (reached) rowClasses.push('reached');
             ...
             const isClickable = isPot || (Boolean(level?.reached) && !Boolean(level?.won) && levelType === 'value_sum');
            if (level?.won) {
              stateClass = 'won';
        ...
        const renderRewardCards = () => {
          ...
          const cardCount = getRewardCardCount();
          if (rewardCardsBoxEl instanceof HTMLElement) {
            const needsScrollableDeck = cardCount > MIN_REWARD_CARD_COUNT;
            rewardCardsBoxEl.style.maxHeight = needsScrollableDeck ? 'min(62vh, 560px)' : '';
            rewardCardsBoxEl.style.overflowY = needsScrollableDeck ? 'auto' : '';
            rewardCardsBoxEl.style.overscrollBehavior = needsScrollableDeck ? 'contain' : '';
            if (needsScrollableDeck) {
              rewardCardsBoxEl.setAttribute('tabindex', '0');
              rewardCardsBoxEl.setAttribute('aria-label', 'کارت‌های جایزه؛ برای مشاهده همه کارت‌ها پیمایش کنید');
            } else {
              rewardCardsBoxEl.removeAttribute('tabindex');
              rewardCardsBoxEl.removeAttribute('aria-label');
            }
          }
          if (!Array.isArray(rewardCardsDeck) || rewardCardsDeck.length !== cardCount) {
        ...
        const refreshRewardState = async ({ force = false } = {}) => {
          ...
          try {
            ...
            if (!rewardCardsStorageLoaded) {
              loadLockedRewardCards();
            } else if (pruneLockedRewardCards()) {
        ...
        const createRewardFlipRequestId = () => {
          try {
          ...
          return 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, (token) => {
            const random = randomNibble();
            const value = token === 'x' ? random : ((random & 0x3) | 0x8);
            return value.toString(16);
          });
        };

        const rememberRewardCardAward = (cardIndex, prizeName) => {
          if (!isValidRewardCardIndex(cardIndex)) return false;
          rewardCardsLockedIndexes.add(cardIndex);
          rewardCardsLockedPrizes.set(cardIndex, String(prizeName || '').trim() || 'جایزه ثبت‌شده');
          saveLockedRewardCards();
          return true;
        };
        ...
        const animateRewardCardsRound = async (pickedButton, prizeName = '') => {
          ...
          const staggerMs = Math.max(90, Math.min(700, Math.floor(5600 / Math.max(1, otherButtons.length))));
          try {
        ...
        const recoverRewardFlipFromServerState = async (levelId, cardIndex) => {
          await refreshRewardState({ force: true });
          const refreshedLevel = getRewardLevelById(levelId);
          if (!refreshedLevel || !Boolean(refreshedLevel?.won)) return '';
          const prizeName = String(refreshedLevel?.wonPrize || '').trim() || 'جایزه ثبت‌شده';
          if (!rememberRewardCardAward(cardIndex, prizeName)) return '';
          renderRewardCards();
          return prizeName;
        };
        ...
        const performRewardFlip = async (cardButton) => {
          ...
          if (!isValidRewardCardIndex(cardIndex)) {
            await openInfoDialog('کارت انتخاب‌شده معتبر نیست. لطفا دوباره تلاش کنید.');
            return;
          }
          ...
          const selectedLevelId = String(selectedRewardLevelId || '').trim();
          const level = getRewardLevelById(selectedLevelId);
          if (!selectedLevelId || !level || String(level?.type || '') !== 'value_sum') {
            await openInfoDialog('سطح جایزه انتخاب‌شده معتبر نیست. لطفا به فهرست جوایز برگردید.');
            return;
          }
          if (Boolean(level?.won)) {
            await openInfoDialog('جایزه این سطح قبلا دریافت شده است.');
            return;
          }
          if (!Boolean(level?.reached) || !Boolean(level?.canFlip)) {
            await openInfoDialog('این سطح هنوز آماده نیست. امتیاز بیشتری جمع کنید.');
            return;
          }
          const requestId = createRewardFlipRequestId();
          ...
          try {
            ...
            const data = payload?.data || {};
            const responseLevelId = String(data.levelId || '').trim();
            if (responseLevelId && responseLevelId !== selectedLevelId) {
              throw new Error('پاسخ جایزه با سطح انتخاب‌شده مطابقت ندارد.');
            }
            const prizeName = String(data.prizeName || '').trim();
            if (!prizeName) {
              throw new Error('نام جایزه در پاسخ سرور موجود نیست.');
            }
            rememberRewardCardAward(cardIndex, prizeName);
            ...
            const recoveredPrizeName = await recoverRewardFlipFromServerState(selectedLevelId, cardIndex);
            if (recoveredPrizeName) {
              await openRewardWinDialog(recoveredPrizeName);
            } else {
        ...
        const continueConditionalQuizAfterReview = () => {
          if (currentTaskType !== 'conditional_quiz' || !conditionalQuizPendingNext) {
            return;
          }
          const { item, answerText, answerPayload } = conditionalQuizPendingNext;
          hideConditionalQuizNextButton();
        ...
        const handleChoiceAnswer = async (button, item, isCorrect, answerText) => {
          ...
          if (wasCorrect) {
            ...
            if (currentTaskType === 'conditional_quiz') {
              showConditionalQuizNextButton(item, answerText, answerPayload);
              return;
            }
            setTimeout(() => {
              if (isQuizAttemptReadyToComplete(answerPayload)) {
        ...
        const handlePercentageAnswer = async (submitButton, slider, item) => {
          ...
          if (currentTaskType === 'conditional_quiz') {
            showConditionalQuizNextButton(item, String(value), answerPayload);
            return;
          }
          setTimeout(() => {
            if (isQuizAttemptReadyToComplete(answerPayload)) {
        ...
        if (quizNextBtnEl instanceof HTMLButtonElement) {
          quizNextBtnEl.addEventListener('click', continueConditionalQuizAfterReview);
        }
