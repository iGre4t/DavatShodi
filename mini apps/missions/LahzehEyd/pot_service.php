...
function tcPotReadWinners(string $levelId): array
{
  ...
  if (!tcDbIsFile($path)) $path = tcPotLegacyWinnersPath($levelId);
  if (!tcDbIsFile($path)) return [];
  $raw = tcDbFileGetContents($path);
...
function tcPotMutateWinners(string $levelId, callable $callback): array
{
  ...
  if ($newStorageIsEmpty && tcDbIsFile(tcPotLegacyWinnersPath($levelId))) {
    $legacyRaw = tcDbFileGetContents(tcPotLegacyWinnersPath($levelId));
