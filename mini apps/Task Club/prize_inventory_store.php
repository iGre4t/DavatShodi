  ...
  if (is_file($path)) {
    ...
    if (is_file($path . '.backup-unsynced') || !$backupMatches) {
      if (!tcPrizeInventoryRefreshBackup($path)) {
        @file_put_contents($path . '.backup-unsynced', tcPrizeInventoryVersion($records) . PHP_EOL, LOCK_EX);
        tcPrizeInventoryEnd($path);
        return null;
      }
...
function tcPrizeInventoryRefreshBackup(string $path): bool
{
  ...
  @unlink($temporaryPath);
  if (!@copy($path, $temporaryPath) || tcPrizeInventoryDecodeFile($temporaryPath) === null) {
    @unlink($temporaryPath);
    return false;
  }
