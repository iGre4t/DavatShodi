...
function egmPrizeLevelsReplacePrepared(string $preparedPath, string $targetPath): bool
{
  if (PHP_OS_FAMILY !== 'Windows') {
    return @egmDbRename($preparedPath, $targetPath);
  }
  ...
  if (@egmDbRename($preparedPath, $targetPath)) {
    @egmDbUnlink($rollbackPath);
    return true;
  }
...
function egmPrizeLevelsRestoreFromFile(string $sourcePath, string $targetPath): bool
{
  ...
  $corruptPath = '';
  if (egmDbIsFile($targetPath)) {
    $corruptPath = egmPrizeLevelsCorruptPath($targetPath);
    if (!@egmDbRename($targetPath, $corruptPath)) {
      @egmDbUnlink($temporaryPath);
      return false;
    }
  }
  ...
  @egmDbUnlink($temporaryPath);
  if ($corruptPath !== '' && egmDbIsFile($corruptPath)) {
    @egmDbRename($corruptPath, $targetPath);
  }
  return false;
}
...
function egmPrizeLevelsRecoverInterruptedReplace(string $path): void
{
  ...
  if (($rollback['ok'] ?? false) && ($rollback['exists'] ?? false)) {
    ...
    if (@egmDbRename($path, $corruptPath)) {
      if (!@egmDbRename($rollbackPath, $path)) {
        @egmDbRename($corruptPath, $path);
      }
    }
  }
}
