...
$tcInviteesSessionUser = requireTabPermissionFromSession('rate-me', false);
if (!userHasPermissionId($tcInviteesSessionUser, 'rate-me:invitees')) {
...
$mapFile = $baseDir . DIRECTORY_SEPARATOR . 'rms Mapped.json';
$stats = [
  'total' => 0,
  'columns' => [],
  'conflicts' => [
...
(() => {
  ...
  const postRevealAction = async (action, payload = {}) => {
    const response = await fetch('mini%20apps/RateMe/invitees_password_guard.php', {
      method: 'POST',
  ...
  editSaveBtn?.addEventListener('click', async () => {
    ...
    try {
      const response = await fetch('mini%20apps/RateMe/invitees_update.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          csrf: csrfToken,
  ...
  uploadBtn?.addEventListener('click', async () => {
    ...
    try {
      ...
      const response = await fetch('mini%20apps/RateMe/invitees_upload.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  addInviteeBtn?.addEventListener('click', async () => {
    ...
    try {
      const response = await fetch('mini%20apps/RateMe/invitees_add.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          csrf: csrfToken,
