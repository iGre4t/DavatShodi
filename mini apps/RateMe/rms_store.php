...
$tcStoreSessionUser = requireTabPermissionFromSession('rate-me', true);
tcSecurityGetCsrfToken();

header('Content-Type: application/json; charset=utf-8');

$baseDir = __DIR__;
$prizesFile = $baseDir . DIRECTORY_SEPARATOR . 'rms Prizes.json';
$prizeLevelsFile = $baseDir . DIRECTORY_SEPARATOR . 'TC Prize Levels.json';
$settingsFile = $baseDir . DIRECTORY_SEPARATOR . 'Setting.json';
$inviteesMappedFile = $baseDir . DIRECTORY_SEPARATOR . 'rms Event' . DIRECTORY_SEPARATOR . 'Invitees mapped.csv';
$inviteesMapFile = $baseDir . DIRECTORY_SEPARATOR . 'rms Event' . DIRECTORY_SEPARATOR . 'rms Mapped.json';
...
if (in_array($action, $tcStoreMainActions, true) && !userHasPermissionId($tcStoreSessionUser, 'rate-me:main')) {
...
if (in_array($action, ['get_settings', 'save_settings'], true)) {
  $canMain = userHasPermissionId($tcStoreSessionUser, 'rate-me:main');
  $canEventStyle = userHasPermissionId($tcStoreSessionUser, 'rate-me:event-style');
