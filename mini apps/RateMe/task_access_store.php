...
$tcTaskAccessSessionUser = requireTabPermissionFromSession('rate-me', true);
if (!userHasPermissionId($tcTaskAccessSessionUser, 'rate-me:task-access')) {
...
function tcTaskAccessLoadPanelUsers(): array
{
  ...
  foreach ($users as $entry) {
    ...
    $permissions = normalizeTabPermissions($entry['permissions'] ?? null, true);
    if (!in_array('rate-me', $permissions, true)) {
      continue;
    }
    $fullName = trim((string)($entry['fullname'] ?? $entry['name'] ?? ''));
    $username = trim((string)($entry['username'] ?? ''));
...
function tcTaskAccessResolveManageTasksPermissionForUser(string $userCode): bool
{
  ...
  if ($pdo instanceof PDO) {
    ...
    if (is_array($user)) {
      return userHasPermissionId($user, 'rate-me:manage-tasks');
    }
  }
  ...
  foreach ($storeUsers as $entry) {
    ...
    $permissions = normalizeTabPermissions($entry['permissions'] ?? null, true);
    return in_array('rate-me:manage-tasks', $permissions, true);
  }
