...
function egmMonitoringExportGregorianToJalali(int $gy, int $gm, int $gd): array
{
...
function egmMonitoringExportShamsiDate(?DateTimeInterface $date, bool $withTime = false): string
{
  if (!$date) {
    return 'نامشخص';
  }
  [$jy, $jm, $jd] = egmMonitoringExportGregorianToJalali(
    (int)$date->format('Y'),
    (int)$date->format('n'),
    (int)$date->format('j')
  );
...
function egmMonitoringExportParseTaskDate(array $task, string $dateKey, string $timeKey, string $defaultTime): ?DateTimeImmutable
{
...
function egmMonitoringExportEventWindow(array $tasks): array
{
  $starts = [];
  $ends = [];
  foreach ($tasks as $task) {
    ...
    $start = egmMonitoringExportParseTaskDate($task, 'startDate', 'startTime', '00:00');
    $end = egmMonitoringExportParseTaskDate($task, 'endDate', 'endTime', '23:59');
    if ($start) {
      $starts[] = $start;
    }
    if ($end) {
...
function egmMonitoringExportEventTitle(string $baseDir): string
{
  $settings = egmMonitoringReadJson($baseDir . DIRECTORY_SEPARATOR . 'Setting.json', []);
  $title = is_array($settings) ? trim((string)($settings['eventName'] ?? '')) : '';
  return $title !== '' ? $title : 'باشگاه ماموریت';
}

function egmMonitoringExportCell($value, string $style = ''): string
{
  $styleAttribute = $style !== '' ? ' ss:StyleID="' . egmMonitoringExportXmlEscape($style) . '"' : '';
  ...
  return '<Cell' . $styleAttribute . '><Data ss:Type="String">'
    . egmMonitoringExportXmlEscape((string)$value)
    . '</Data></Cell>';
}

function egmMonitoringExportWorksheet(string $name, array $headers, array $rows, array $widths = []): string
{
  ...
  $xml = '<Worksheet ss:Name="' . egmMonitoringExportXmlEscape($name) . '"><Table>';
  ...
  if ($headers) {
    ...
    foreach ($headers as $header) {
      $xml .= egmMonitoringExportCell((string)$header, 'sHeader');
    }
    $xml .= '</Row>';
  }
  foreach ($rows as $row) {
    ...
    foreach ((array)$row as $cell) {
      if (is_array($cell) && array_key_exists('value', $cell)) {
        $xml .= egmMonitoringExportCell($cell['value'], (string)($cell['style'] ?? ''));
      } else {
        $xml .= egmMonitoringExportCell($cell);
      }
    }
    $xml .= '</Row>';
  }
...
function egmMonitoringExportPercent(float $percent): array
{
  return ['value' => max(0.0, $percent) / 100, 'style' => 'sPercent'];
}

function egmMonitoringExportLevelType(string $type): string
{
...
function egmMonitoringBuildExcelXml(array $data, string $baseDir): array
{
  $tasks = egmMonitoringReadTasks(
    $baseDir . DIRECTORY_SEPARATOR . 'tasks' . DIRECTORY_SEPARATOR . 'tasks.js',
    $baseDir . DIRECTORY_SEPARATOR . 'tasks'
  );
  $window = egmMonitoringExportEventWindow($tasks);
  $eventTitle = egmMonitoringExportEventTitle($baseDir);
  ...
  $sheets = [];
  $sheets[] = egmMonitoringExportWorksheet('اطلاعات رویداد', ['عنوان', 'مقدار'], [
    ['نام رویداد', $eventTitle],
    ['شروع رویداد', egmMonitoringExportShamsiDate($window['start'] ?? null, true)],
    ['پایان رویداد', egmMonitoringExportShamsiDate($window['end'] ?? null, true)],
    ['مدت رویداد', (string)($window['duration'] ?? 'نامشخص')],
    ['تاریخ تهیه گزارش', egmMonitoringExportShamsiDate($exportedAt, true)],
  ...
  $sheets[] = egmMonitoringExportWorksheet('خلاصه مانیتورینگ', ['شاخص', 'مقدار'], $summaryRows, [280, 120]);

  $participationRows = [
    ['کل دعوت‌شدگان', $totalUsers, egmMonitoringExportPercent($totalUsers > 0 ? 100 : 0), 'همه کاربران گروه‌های انتخاب‌شده'],
    ['حداقل یک‌بار ورود', $loggedInUsers, egmMonitoringExportPercent($totalUsers > 0 ? ($loggedInUsers * 100 / $totalUsers) : 0), 'نرخ ورود از کل دعوت‌شدگان'],
    ['مشارکت در حداقل یک ماموریت', (int)($participation['usersWithTaskParticipation'] ?? 0), egmMonitoringExportPercent($totalUsers > 0 ? ((int)($participation['usersWithTaskParticipation'] ?? 0) * 100 / $totalUsers) : 0), 'تعامل یا تکمیل حداقل یک ماموریت'],
    ['تکمیل همه ماموریت‌ها', (int)($participation['usersCompletedAllStarted'] ?? 0), egmMonitoringExportPercent($totalUsers > 0 ? ((int)($participation['usersCompletedAllStarted'] ?? 0) * 100 / $totalUsers) : 0), 'تکمیل همه ماموریت‌های شروع‌شده'],
    ['بالاترین امتیاز ممکن', (int)($participation['usersWithMaxPossibleScore'] ?? 0), egmMonitoringExportPercent($totalUsers > 0 ? ((int)($participation['usersWithMaxPossibleScore'] ?? 0) * 100 / $totalUsers) : 0), 'رسیدن به حداکثر امتیاز قابل دریافت'],
    ['واجد سطح جایزه', (int)($participation['usersEligibleAnyLevel'] ?? 0), egmMonitoringExportPercent($totalUsers > 0 ? ((int)($participation['usersEligibleAnyLevel'] ?? 0) * 100 / $totalUsers) : 0), 'رسیدن به حداقل یک سطح جایزه'],
    ['نرخ ترک زودهنگام', (int)($participation['loggedInWithoutCompletion'] ?? 0), egmMonitoringExportPercent($loggedInUsers > 0 ? ((int)($participation['loggedInWithoutCompletion'] ?? 0) * 100 / $loggedInUsers) : 0), 'وارد شده‌اند اما هیچ ماموریتی را تکمیل نکرده‌اند'],
    ['بدون ورود', (int)($participation['neverLoggedInUsers'] ?? 0), egmMonitoringExportPercent($totalUsers > 0 ? ((int)($participation['neverLoggedInUsers'] ?? 0) * 100 / $totalUsers) : 0), 'دعوت‌شدگانی که وارد نشده‌اند']
  ];
  $sheets[] = egmMonitoringExportWorksheet('شاخص‌های مشارکت', ['شاخص', 'تعداد', 'نرخ', 'تعریف'], $participationRows, [230, 90, 90, 360]);
  ...
  foreach ((array)($data['taskStats'] ?? []) as $task) {
    ...
    $start = is_array($sourceTask) ? egmMonitoringExportParseTaskDate($sourceTask, 'startDate', 'startTime', '00:00') : null;
    $end = is_array($sourceTask) ? egmMonitoringExportParseTaskDate($sourceTask, 'endDate', 'endTime', '23:59') : null;
    $taskRows[] = [
      (string)($task['title'] ?? ''),
      (string)($task['tagCode'] ?? ''),
      egmMonitoringExportShamsiDate($start, true),
      egmMonitoringExportShamsiDate($end, true),
      (int)($task['completedUsers'] ?? 0),
      egmMonitoringExportPercent((float)($task['completionRate'] ?? 0)),
      (float)($task['awardedScoreAvg'] ?? 0),
      (float)($task['awardedScoreTotal'] ?? 0)
    ];
  }
  $sheets[] = egmMonitoringExportWorksheet('ماموریت‌ها', ['ماموریت', 'کد', 'شروع', 'پایان', 'تکمیل‌شده', 'نرخ تکمیل', 'میانگین امتیاز', 'مجموع امتیاز'], $taskRows, [260, 100, 170, 170, 95, 95, 110, 110]);

  $levelRows = [];
  foreach ((array)($data['levelStats'] ?? []) as $level) {
    $levelRows[] = [(string)($level['name'] ?? ''), egmMonitoringExportLevelType((string)($level['type'] ?? '')), (int)($level['score'] ?? 0), (int)($level['eligibleUsers'] ?? 0), egmMonitoringExportPercent((float)($level['eligibleRate'] ?? 0))];
  }
  $sheets[] = egmMonitoringExportWorksheet('سطوح جایزه', ['نام سطح', 'نوع', 'امتیاز لازم', 'کاربران واجد', 'نرخ واجد بودن'], $levelRows, [220, 120, 110, 110, 110]);
  ...
  foreach ((array)($data['scoreStages'] ?? []) as $stage) {
    ...
    $scoreRows[] = [$label, (int)($stage['users'] ?? 0), egmMonitoringExportPercent((float)($stage['percentage'] ?? 0)), (float)($stage['avgScore'] ?? 0)];
  }
  $sheets[] = egmMonitoringExportWorksheet('پراکندگی امتیاز', ['بازه', 'کاربران', 'درصد', 'میانگین امتیاز'], $scoreRows, [240, 100, 90, 120]);

  $groupRows = [];
  foreach ((array)($data['workIdGroupStats'] ?? []) as $group) {
    $groupRows[] = [(string)($group['group'] ?? ''), (int)($group['users'] ?? 0), (int)($group['participants'] ?? 0), egmMonitoringExportPercent((float)($group['participantRate'] ?? 0)), (float)($group['avgCompletedStartedMissions'] ?? 0), (float)($group['avgScore'] ?? 0)];
  }
  $sheets[] = egmMonitoringExportWorksheet('گروه‌های پرسنلی', ['گروه', 'کاربران', 'مشارکت‌کننده', 'نرخ مشارکت', 'میانگین ماموریت', 'میانگین امتیاز'], $groupRows, [180, 90, 110, 100, 120, 110]);
  ...
  $sheets[] = egmMonitoringExportWorksheet('فعال‌ترین کاربران', ['رتبه', 'نام', 'شماره پرسنلی', 'امتیاز', 'تعداد ورود', 'ماموریت تکمیل‌شده', 'شاخص فعالیت'], $activeRows, [70, 220, 120, 90, 90, 130, 100]);
  ...
  $sheets[] = egmMonitoringExportWorksheet('موجودی جوایز', ['جایزه', 'ظرفیت', 'باقی‌مانده', 'داده‌شده', 'ارزش هر مورد'], $prizeRows, [260, 90, 100, 100, 120]);
  ...
  $xml = '<?xml version="1.0" encoding="UTF-8"?>'
    ...
    . egmMonitoringExportXmlEscape('گزارش مانیتورینگ ' . $eventTitle)
...
function egmMonitoringSendExcelExport(array $data, string $baseDir): void
{
  ...
  $export = egmMonitoringBuildExcelXml($data, $baseDir);
