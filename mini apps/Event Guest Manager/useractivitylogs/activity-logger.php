...
function egmActivityWriteJsonLine(array $entry): bool
{
    try {
        ...
        if (!is_dir($directory) && !mkdir($directory, 0755, true) && !is_dir($directory)) {
            error_log('Event Guest Manager activity logger failed to create log directory: ' . $directory);
            return false;
        }
        $line = json_encode($entry, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_INVALID_UTF8_SUBSTITUTE);
        if ($line === false) {
            error_log('Event Guest Manager activity logger failed to encode JSON: ' . json_last_error_msg());
            return false;
        }
        $path = egmActivityLogFilePath($entry);
        $handle = fopen($path, 'ab');
        if ($handle === false) {
            error_log('Event Guest Manager activity logger failed to open log file: ' . $path);
            return false;
        }
        $written = false;
        ...
        fclose($handle);
        if (!$written) {
            error_log('Event Guest Manager activity logger failed to write log line: ' . $path);
        }
        return $written;
    } catch (Throwable $err) {
        error_log('Event Guest Manager activity logger file write failed: ' . $err->getMessage());
        return false;
    }
}
