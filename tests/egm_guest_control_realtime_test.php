...
egmRealtimeAssert(
    str_contains($source, 'function egmCheckInDashboardStats(')
        ...
        && str_contains($source, 'از <b data-stat="total">۰</b> نفر دعوت‌شده')
        && str_contains($source, 'data-stat="invited_entered"')
        && str_contains($source, 'data-stat="other_period_entered"')
        && str_contains($source, 'data-stat="walk_in_entered"')
        && str_contains($source, 'entry-composition-item other-period')
        && str_contains($source, 'entry-composition-item walk-in')
        && !str_contains($source, '<div class="stat-tile walk-in">')
