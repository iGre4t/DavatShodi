...
function getEventGuestsPath(array $event, string $eventsRoot): string
{
    ...
    return rtrim($eventDir, '/\\') . DIRECTORY_SEPARATOR . 'eventguests.json';
}

function loadEventGuestsFromFile(string $path): array
{
    if (!is_file($path)) {
        return [];
    }
...
function saveEventGuestsToFile(string $path, array $guests): bool
{
    $dir = dirname($path);
    if (!is_dir($dir) && !mkdir($dir, 0755, true) && !is_dir($dir)) {
        return false;
    }
    $encoded = json_encode(array_values($guests), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    if ($encoded === false) {
        return false;
    }
...
function loadEventGuestsForStore(array $event, string $eventsRoot, bool $persistIfMissing = false): array
{
    $path = getEventGuestsPath($event, $eventsRoot);
    if (is_file($path)) {
        return loadEventGuestsFromFile($path);
    }
    $legacyGuests = is_array($event['guests'] ?? null) ? array_values($event['guests']) : [];
    if ($persistIfMissing && !empty($legacyGuests)) {
...
if ($method === 'POST') {
    ...
    if ($action === 'scan_invite') {
        ...
        if ($match === null) {
            ...
            if (!saveGuestStore($storePath, $store, $eventsRoot)) {
                http_response_code(500);
...
function saveGuestStore(string $path, array $data, string $eventsRoot = ''): bool
{
    ...
    $metadata = $data;
    $metadata['events'] = array_values(array_map(static function ($event) {
        if (!is_array($event)) {
            return [];
        }
        $event['guest_count'] = (int)($event['guest_count'] ?? (is_array($event['guests'] ?? null) ? count($event['guests']) : 0));
        unset($event['guests']);
        return $event;
    }, $metadata['events'] ?? []));
    $dir = dirname($path);
...
function normalizeStore(array $store, string $eventsRoot = '', bool $loadEventGuests = true): array
{
    ...
    foreach ($store['events'] as &$event) {
        ...
        if ($loadEventGuests && $eventsRoot !== '') {
            $event['guests'] = loadEventGuestsForStore($event, $eventsRoot, true);
        } else {
