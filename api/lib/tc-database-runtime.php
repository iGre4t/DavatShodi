 ...
 *
 * TC's legacy application surface still expresses several domain documents
 * as paths. These helpers keep that API stable while storing the bytes in the
 * per-instance InnoDB data table. Non-TC/code/upload paths always fall back
 * to native filesystem operations.
 */

function &tcDatabaseRuntimeContextCache(): array
{
    static $cache = [];
    return $cache;
}
...
function tcDatabaseRuntimeContextForPath(string $path): ?array
{
    ...
    $cache =& tcDatabaseRuntimeContextCache();
    ...
    foreach ($cache as $rootKey => $cached) {
        if ($casePath === $rootKey || str_starts_with($casePath, $rootKey . '/')) {
            ...
            $relative = tcInstanceNormalizeRuntimeRelativePath($relative);
            if ($relative !== '' && tcInstanceIsManagedRuntimeFile($relative)) return $cached + ['relative' => $relative];
            return null;
        }
    }

    // Reject non-TC paths before touching the filesystem. Walking ancestors
    ...
    $missionRoot = '';
    if (preg_match('#^(.*/mini apps/Task Club)(?:/|$)#i', $absolute, $match) === 1) {
        $missionRoot = rtrim((string)$match[1], '/');
    } elseif (preg_match('#^(.*/mini apps/missions/[^/]+)(?:/|$)#i', $absolute, $match) === 1) {
        $missionRoot = rtrim((string)$match[1], '/');
    }
    if ($missionRoot === ''
        || !is_dir(str_replace('/', DIRECTORY_SEPARATOR, $missionRoot))
        || tcInstanceRegistryDirectoryForMission(str_replace('/', DIRECTORY_SEPARATOR, $missionRoot)) === '') {
        return null;
    }
    $relative = tcInstanceNormalizeRuntimeRelativePath(ltrim(substr($absolute, strlen($missionRoot)), '/'));
    if ($relative === '' || !tcInstanceIsManagedRuntimeFile($relative)) return null;
...
function tcDatabaseRuntimeVirtualCsvKind(array $context): string
{
...
function tcDatabaseRuntimeCsvEncode(array $rows): string
{
    $stream = fopen('php://temp/maxmemory:8388608', 'w+b');
    if (!is_resource($stream)) throw new RuntimeException('Unable to prepare the TC compatibility view.');
    foreach ($rows as $row) {
        if (fputcsv($stream, is_array($row) ? $row : [], ',', '"', '\\') === false) {
            fclose($stream);
            throw new RuntimeException('Unable to encode the TC compatibility view.');
        }
    }
    rewind($stream);
    $content = stream_get_contents($stream);
    fclose($stream);
    if (!is_string($content)) throw new RuntimeException('Unable to read the TC compatibility view.');
    return $content;
}

function tcDatabaseRuntimeCsvDecode(string $content): array
{
    $stream = fopen('php://temp/maxmemory:8388608', 'w+b');
...
function tcDatabaseRuntimeNormalizeHeader(string $value): string
{
...
/** @return list<string> */
function tcDatabaseRuntimeInviteeHeader(): array
{
    return [
        ...
        'prize won at', 'wheel angle', 'invitees', 'answers', 'inner score', 'score', 'Answered',
...
function tcDatabaseRuntimePeriodCode(array $context): string
{
...
function tcDatabaseRuntimeDefaultInviteeMapping(): array
{
    return [
        'workId' => 0,
        'firstName' => 1,
...
function tcDatabaseRuntimeStructuredDocumentRead(array $context): ?array
{
    $relative = strtolower(str_replace('\\', '/', (string)$context['relative']));
    if (str_ends_with($relative, '/response-results.json')) {
        $periodCode = tcDatabaseRuntimePeriodCode($context);
        $statement = $context['pdo']->prepare(
            "SELECT `work_id`,`payload_json`,UNIX_TIMESTAMP(`updated_at`) AS `mtime` "
            . "FROM `{$context['tables']['shared_answers_quiz_results']}` WHERE `period_code`=:period ORDER BY `id`"
        );
        ...
        foreach ($statement->fetchAll(PDO::FETCH_ASSOC) as $row) {
            ...
            $result = json_decode((string)($row['payload_json'] ?? ''), true);
            if ($workId !== '' && is_array($result)) $payload[$workId] = $result;
            $mtime = max($mtime, (int)($row['mtime'] ?? 0));
        }
        $content = json_encode($payload, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        if (!is_string($content)) throw new RuntimeException('Unable to encode shared answers quiz results.');
        $content .= PHP_EOL;
        return ['content' => $content, 'size' => strlen($content), 'mtime' => $mtime ?: time()] + $context;
    }
    if ($relative !== 'tc event/tc mapped.json') return null;
    $payload = tcInstanceReadData($context['pdo'], $context['code'], 'invitee_mapping', null);
    if (!is_array($payload)) {
        $payload = tcDatabaseRuntimeDefaultInviteeMapping();
        tcInstanceWriteData($context['pdo'], $context['code'], 'invitee_mapping', $payload);
    }
    $content = json_encode($payload, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    if (!is_string($content)) throw new RuntimeException('Unable to encode the TC invitee mapping.');
    return ['content' => $content . PHP_EOL, 'size' => strlen($content . PHP_EOL), 'mtime' => time()] + $context;
}

function tcDatabaseRuntimeStructuredDocumentWrite(array $context, string $content): bool
{
    $relative = strtolower(str_replace('\\', '/', (string)$context['relative']));
    if (str_ends_with($relative, '/response-results.json')) {
        $decoded = json_decode($content, true);
        if (!is_array($decoded)) throw new RuntimeException('The shared answers quiz result document is invalid.');
        $periodCode = tcDatabaseRuntimePeriodCode($context);
        $pdo = $context['pdo'];
        $table = $context['tables']['shared_answers_quiz_results'];
        $users = tcInstanceUserIdMaps($pdo, $context['tables']['users']);
        $ownsTransaction = !$pdo->inTransaction();
        if ($ownsTransaction) $pdo->beginTransaction();
        try {
            $pdo->prepare("DELETE FROM `{$table}` WHERE `period_code`=:period")->execute([':period' => $periodCode]);
            $insert = $pdo->prepare(
                "INSERT INTO `{$table}` (`user_id`,`work_id`,`period_code`,`inner_score`,`level_id`,`level_name`,`response_text`,`start_score`,`end_score`,`payload_json`,`completed_at`) "
            ...
            foreach ($decoded as $workIdRaw => $result) {
                ...
                $insert->execute([
                    ':user_id' => tcInstanceResolveUserId($users, $workId), ':work_id' => $workId,
                    ':period_code' => $periodCode, ':inner_score' => (int)($result['innerScore'] ?? 0),
            ...
            tcDatabaseRuntimeRemoveStoredFileRows($context);
    ...
    if ($relative !== 'tc event/tc mapped.json') return false;
    $decoded = json_decode($content, true);
    if (!is_array($decoded)) throw new RuntimeException('The TC invitee mapping is invalid.');
    tcInstanceWriteData($context['pdo'], $context['code'], 'invitee_mapping', $decoded);
    tcDatabaseRuntimeRemoveStoredFileRows($context);
    tcDatabaseRuntimeRefreshManifest($context);
    return true;
}

function tcDatabaseRuntimeInviteeRows(array $context): array
{
    $header = tcDatabaseRuntimeInviteeHeader();
    $headerLookup = [];
    foreach ($header as $index => $name) $headerLookup[tcDatabaseRuntimeNormalizeHeader($name)] = $index;
    $periodCode = tcDatabaseRuntimePeriodCode($context);
    ...
    foreach ($users as $user) {
        ...
        foreach ([$excel, $state, $direct] as $source) {
            foreach ($source as $key => $value) {
                $normalized = tcDatabaseRuntimeNormalizeHeader((string)$key);
...
function tcDatabaseRuntimeAnswersRows(array $context): array
{
    $periodCode = tcDatabaseRuntimePeriodCode($context);
    $statement = $context['pdo']->prepare(
...
function tcDatabaseRuntimeVirtualCsvRead(array $context): ?array
{
    $kind = tcDatabaseRuntimeVirtualCsvKind($context);
    if ($kind === '') return null;
    $rows = $kind === 'invitees' ? tcDatabaseRuntimeInviteeRows($context) : tcDatabaseRuntimeAnswersRows($context);
    $content = tcDatabaseRuntimeCsvEncode($rows);
    $tables = $context['tables'];
    $table = $kind === 'invitees' ? $tables['users'] : $tables['answers'];
    $updatedColumn = $kind === 'invitees' ? 'tc_updated_at' : 'updated_at';
    $mtime = (int)$context['pdo']->query("SELECT COALESCE(UNIX_TIMESTAMP(MAX(`{$updatedColumn}`)),0) FROM `{$table}`")->fetchColumn();
    return ['content' => $content, 'size' => strlen($content), 'mtime' => $mtime ?: time()] + $context;
}

function tcDatabaseRuntimeHeaderIndex(array $header): array
{
    $indexes = [];
    foreach ($header as $index => $name) $indexes[tcDatabaseRuntimeNormalizeHeader((string)$name)] = (int)$index;
    return $indexes;
}
...
function tcDatabaseRuntimeWriteInvitees(array $context, array $rows): void
{
    ...
    $indexes = tcDatabaseRuntimeHeaderIndex($header);
    $workIndex = $indexes['work id'] ?? $indexes['workid'] ?? -1;
    if ($workIndex < 0) throw new RuntimeException('The TC participant table has no Work ID column.');
    $periodCode = tcDatabaseRuntimePeriodCode($context);
    ...
    try {
        ...
        foreach (array_slice($rows, 1) as $offset => $row) {
            ...
            foreach ($header as $index => $name) {
                $key = tcDatabaseRuntimeNormalizeHeader((string)$name);
        ...
        tcInstanceAssignGuestNumbers($pdo, $context['code']);
        if ($ownsTransaction) $pdo->commit();
    } catch (Throwable $error) {
        if ($ownsTransaction && $pdo->inTransaction()) $pdo->rollBack();
        throw $error;
...
function tcDatabaseRuntimeWriteAnswers(array $context, array $rows): void
{
    if (!$rows || !is_array($rows[0] ?? null)) throw new RuntimeException('The TC answers table is empty.');
    $header = array_map('strval', $rows[0]);
    $indexes = tcDatabaseRuntimeHeaderIndex($header);
    $workIndex = $indexes['work id'] ?? $indexes['workid'] ?? -1;
    if ($workIndex < 0) throw new RuntimeException('The TC answers table has no Work ID column.');
    $pdo = $context['pdo'];
    $periodCode = tcDatabaseRuntimePeriodCode($context);
    $users = tcInstanceUserIdMaps($pdo, $context['tables']['users']);
    $ownsTransaction = !$pdo->inTransaction();
    ...
    try {
        ...
        foreach (array_slice($rows, 1) as $row) {
            $workId = trim((string)($row[$workIndex] ?? ''));
            $userId = tcInstanceResolveUserId($users, $workId);
...
function tcDatabaseRuntimeRemoveStoredFileRows(array $context): void
{
    ...
    $context['pdo']->prepare("DELETE FROM `{$table}` WHERE (`storage_kind`='runtime_chunk' AND `file_path`=:path) OR (`storage_kind`='runtime_file' AND `data_key`=:key)")
        ->execute([':path' => $context['relative'], ':key' => tcInstanceRuntimeFileDataKey($context['relative'])]);
}

function tcDatabaseRuntimeVirtualCsvWrite(array $context, string $content): bool
{
    $kind = tcDatabaseRuntimeVirtualCsvKind($context);
    if ($kind === '') return false;
    $rows = tcDatabaseRuntimeCsvDecode($content);
    if ($kind === 'invitees') tcDatabaseRuntimeWriteInvitees($context, $rows);
    else tcDatabaseRuntimeWriteAnswers($context, $rows);
    tcDatabaseRuntimeRemoveStoredFileRows($context);
    tcDatabaseRuntimeRefreshManifest($context);
    return true;
}
 ...
 */
function tcDatabaseRuntimeVerifyUserPassword(string $missionDir, string $workId, string $plainPassword): ?bool
{
    $context = tcDatabaseRuntimeContextForPath(rtrim($missionDir, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'TC Event' . DIRECTORY_SEPARATOR . 'Invitees mapped.csv');
...
function tcDatabaseRuntimeActivityEntries(string $missionDir, string $purpose = 'all'): array
{
    ...
    if ($purpose === 'completion') {
        $where = " WHERE `action`='taskclub.task.action' "
            . "AND JSON_UNQUOTE(JSON_EXTRACT(`metadata_json`,'$.request_action'))='quiz_complete_finished'";
    } elseif ($purpose === 'engagement') {
    ...
    return tcDatabaseRuntimeActivityRowsToEntries($rows);
}

/** @param list<array<string,mixed>> $rows
 *  @return list<array<string,mixed>>
 */
function tcDatabaseRuntimeActivityRowsToEntries(array $rows): array
{
    $entries = [];
    foreach ($rows as $row) {
        ...
        $entries[] = [
            ...
            'action' => (string)($row['action'] ?? 'taskclub.unspecified'),
...
/** @return list<string> */
function tcDatabaseRuntimeActivityDays(string $missionDir): array
{
    $context = tcDatabaseRuntimeContextForPath(rtrim($missionDir, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'Setting.json');
    if ($context === null) return [];
    $rows = $context['logs_pdo']->query(
...
/** @return list<array<string,mixed>> */
function tcDatabaseRuntimeActivityEntriesForDay(string $missionDir, string $day, int $limit = 2000): array
{
    ...
    $statement->execute([':start' => $day . ' 00:00:00', ':end' => date('Y-m-d H:i:s', strtotime($day . ' +1 day'))]);
    return tcDatabaseRuntimeActivityRowsToEntries($statement->fetchAll(PDO::FETCH_ASSOC));
}

function tcDatabaseRuntimeAppendActivity(string $missionDir, array $entry): bool
{
    $context = tcDatabaseRuntimeContextForPath(rtrim($missionDir, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'Setting.json');
    if ($context === null) return false;
    $workId = trim((string)($entry['user_id'] ?? ''));
    $maps = tcInstanceUserIdMaps($context['pdo'], $context['tables']['users']);
    $metadata = json_encode(is_array($entry['metadata'] ?? null) ? $entry['metadata'] : [], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    $timestamp = tcInstanceDateTimeValue($entry['timestamp'] ?? null) ?? date('Y-m-d H:i:s');
    $source = hash('sha256', implode('|', [
    ...
    return $statement->execute([
        ':source_key' => $source, ':user_id' => tcInstanceResolveUserId($maps, $workId), ':work_id' => $workId ?: null,
        ...
        ':metadata_json' => is_string($metadata) ? $metadata : '{}', ':occurred_at' => $timestamp,
    ]);
}

function tcDatabaseRuntimeRead(string $path): ?array
{
    $context = tcDatabaseRuntimeContextForPath($path);
    if ($context === null) return null;
    $structured = tcDatabaseRuntimeStructuredDocumentRead($context);
    if ($structured !== null) return $structured;
    $virtual = tcDatabaseRuntimeVirtualCsvRead($context);
    ...
    $statement->execute([':key' => tcInstanceRuntimeFileDataKey($context['relative'])]);
    ...
    if (!is_string($content)) {
        ...
        while ($chunkRow = $chunks->fetch(PDO::FETCH_ASSOC)) {
            ...
            if (!is_string($chunk) || hash('sha256', $chunk) !== trim((string)($chunkRow['content_sha256'] ?? ''))) {
                throw new RuntimeException('Corrupt TC database document chunk: ' . $context['relative']);
            }
            $content .= $chunk;
        }
    }
    ...
    if (strlen($content) !== $expectedSize || ($expectedHash !== '' && hash('sha256', $content) !== $expectedHash)) {
        throw new RuntimeException('TC database document failed integrity validation: ' . $context['relative']);
    }
    return ['content' => $content, 'size' => $expectedSize, 'mtime' => (int)($row['file_mtime'] ?? 0)] + $context;
}

function tcDatabaseRuntimeWrite(string $path, string $content, ?int $mtime = null): bool
{
    $context = tcDatabaseRuntimeContextForPath($path);
    if ($context === null) return false;
    ...
    if (strtolower(str_replace('\\', '/', $context['relative'])) === 'tc event/tc mapped.json') {
        return tcDatabaseRuntimeStructuredDocumentWrite($context, $content);
    }
    if (tcDatabaseRuntimeVirtualCsvKind($context) !== '') {
        return tcDatabaseRuntimeVirtualCsvWrite($context, $content);
    }
    $pdo = $context['pdo'];
    $table = $context['tables']['data'];
    ...
    try {
        ...
        $header->execute([':key' => tcInstanceRuntimeFileDataKey($relative), ':payload' => $metadata ?: '{}', ':path' => $relative, ':hash' => $hash, ':size' => strlen($content), ':mtime' => $mtime]);
...
function tcDatabaseRuntimeRefreshManifest(array $context): void
{
    $table = $context['tables']['data'];
    ...
    tcInstanceWriteData($context['pdo'], $context['code'], 'runtime_file_manifest', $manifest);
}

function tcDatabaseRuntimeDelete(string $path): bool
{
    $context = tcDatabaseRuntimeContextForPath($path);
    if ($context === null) return false;
    $pdo = $context['pdo'];
    ...
    try {
        ...
        $delete->execute([':key' => tcInstanceRuntimeFileDataKey($context['relative'])]);
        tcDatabaseRuntimeRefreshManifest($context);
        if ($ownsTransaction) $pdo->commit();
...
function tcDatabaseRuntimePaths(array $context): array
{
    $table = $context['tables']['data'];
...
function tcDbFileGetContents(string $filename, bool $useIncludePath = false, $context = null, int $offset = 0, ?int $length = null)
{
    $row = tcDatabaseRuntimeRead($filename);
...
function tcDbFilePutContents(string $filename, $data, int $flags = 0, $context = null)
{
    if (tcDatabaseRuntimeContextForPath($filename) === null) return file_put_contents($filename, $data, $flags, $context);
    ...
    if (($flags & FILE_APPEND) === FILE_APPEND) {
        $existing = tcDatabaseRuntimeRead($filename);
        $content = (string)($existing['content'] ?? '') . $content;
    }
    tcDatabaseRuntimeWrite($filename, $content);
    return strlen($content);
}

function tcDbIsFile(string $filename): bool
{
    if (tcDatabaseRuntimeContextForPath($filename) !== null) return tcDatabaseRuntimeRead($filename) !== null;
    return is_file($filename);
}

function tcDbFileExists(string $filename): bool
{
    if (tcDatabaseRuntimeContextForPath($filename) !== null) return tcDatabaseRuntimeRead($filename) !== null;
    return file_exists($filename);
}

function tcDbFilemtime(string $filename)
{
    $row = tcDatabaseRuntimeRead($filename);
    return $row === null ? filemtime($filename) : $row['mtime'];
}

function tcDbFilesize(string $filename)
{
    $row = tcDatabaseRuntimeRead($filename);
    return $row === null ? filesize($filename) : $row['size'];
}

function tcDbUnlink(string $filename, $context = null): bool
{
    if (tcDatabaseRuntimeContextForPath($filename) !== null) return tcDatabaseRuntimeDelete($filename);
    return unlink($filename, $context);
}

function tcDbRename(string $from, string $to, $context = null): bool
{
    $source = tcDatabaseRuntimeRead($from);
    $targetManaged = tcDatabaseRuntimeContextForPath($to) !== null;
    if ($source !== null || $targetManaged) {
        $content = $source !== null ? $source['content'] : file_get_contents($from);
        if (!is_string($content)) return false;
        if ($targetManaged) tcDatabaseRuntimeWrite($to, $content, $source['mtime'] ?? time());
        elseif (file_put_contents($to, $content, LOCK_EX) === false) return false;
        if ($source !== null) tcDatabaseRuntimeDelete($from); else @unlink($from);
        return true;
    }
    return rename($from, $to, $context);
}

function tcDbCopy(string $from, string $to, $context = null): bool
{
    $source = tcDatabaseRuntimeRead($from);
    $targetManaged = tcDatabaseRuntimeContextForPath($to) !== null;
    if ($source !== null || $targetManaged) {
        ...
        return $targetManaged
            ? tcDatabaseRuntimeWrite($to, $content, $source['mtime'] ?? time())
            : file_put_contents($to, $content, LOCK_EX) !== false;
    }
    return copy($from, $to, $context);
}

function tcDbGlob(string $pattern, int $flags = 0): array|false
{
    $probe = preg_replace('/[*?\[\]{}].*$/', '', $pattern) ?: $pattern;
    $context = tcDatabaseRuntimeContextForPath(rtrim($probe, "\\/"));
    if ($context === null) return glob($pattern, $flags);
    $normalizedPattern = str_replace('\\', '/', tcDatabaseRuntimeNormalizeAbsolute($pattern));
    $matches = [];
    foreach (tcDatabaseRuntimePaths($context) as $relative) {
...
final class TcDatabaseStreamWrapper
{
    public $context;
    private $buffer;
    private string $path = '';
    ...
    public function stream_open(string $path, string $mode, int $options, ?string &$openedPath): bool
    {
        ...
        $registry =& tcDatabaseStreamRegistry();
        ...
        $existing = tcDatabaseRuntimeRead($this->path);
    ...
    private function persist(): bool
    {
        ...
        if (!is_string($content)) return false;
        $ok = tcDatabaseRuntimeWrite($this->path, $content);
        if ($ok) $this->dirty = false;
        return $ok;
    }
}

function &tcDatabaseStreamRegistry(): array
{
    static $registry = [];
    return $registry;
}

function tcDbFopen(string $filename, string $mode, bool $useIncludePath = false, $context = null)
{
    $databaseContext = tcDatabaseRuntimeContextForPath($filename);
    $lockContext = null;
    if ($databaseContext === null && str_ends_with(strtolower($filename), '.lock')) {
        $lockContext = tcDatabaseRuntimeContextForPath(substr($filename, 0, -5));
    }
    ...
    if (!in_array('tcdb', stream_get_wrappers(), true)) stream_wrapper_register('tcdb', TcDatabaseStreamWrapper::class);
