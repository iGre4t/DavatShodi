...
<head>
  ...
  <style nonce="<?= $nonce ?>">
    ...
    .guest-control-page .table-wrap{overflow:hidden;border:1px solid var(--border,#e5e7eb);border-radius:12px;background:#fff}
    ...
    .guest-control-page th{border-bottom:1px solid #d9dee6;background:#f6f7f9;color:#667085;font-size:11px;font-weight:800}
    ...
    .guest-control-page tbody tr{transition:filter .15s ease}
    .guest-control-page tbody tr:hover{filter:brightness(.985)}
    .guest-control-page tbody tr:last-child td{border-bottom:0}
    .guest-control-page .log-summary-row td{padding-block:11px 7px;border:0}
    .guest-control-page .log-detail-row td{padding:0 10px 9px;border-bottom:2px solid #d7dce4}
    .guest-control-page .log-record-even td{background:#fff}.guest-control-page .log-record-odd td{background:#f8fafc}
    .guest-control-page .log-summary-row.status-success td:first-child{border-inline-start:5px solid #18a566}.guest-control-page .log-summary-row.status-duplicate td:first-child{border-inline-start:5px solid #d59a12}.guest-control-page .log-summary-row.status-error td:first-child{border-inline-start:5px solid #df4052}
    ...
    .guest-control-page .log-detail-line{display:flex;align-items:center;gap:8px 18px;min-width:0;padding-top:7px;border-top:1px dashed #d9dee6;color:#667085;font-size:11px;line-height:1.65}
    ...
    @media(max-width:760px){.guest-control-page .guest-shell{width:min(100% - 20px,1180px);padding:14px 0 28px}.guest-control-page .guest-card{padding:16px 8px}.guest-control-page .hero-head{display:block}.guest-control-page .phase{margin-top:12px}.guest-control-page .period-navigation{grid-template-columns:1fr}.guest-control-page .scan-grid{grid-template-columns:1fr;gap:16px}.guest-control-page .scanner input{height:54px;font-size:24px}.guest-control-page .card-head{align-items:flex-start}.guest-control-page .card-head .muted{display:none}.guest-control-page .log-toolbar{grid-template-columns:1fr}.guest-control-page .log-search-meta{padding:0;text-align:right}.guest-control-page th,.guest-control-page td{padding-inline:5px;font-size:10px}.guest-control-page th{font-size:9px}.guest-control-page .log-detail-line{align-items:flex-start;gap:4px 10px;flex-wrap:wrap}.guest-control-page .log-main-message{flex-basis:100%}.guest-control-page .detail-grid,.guest-control-page .walk-in-grid{grid-template-columns:1fr}.guest-control-page .detail-item.full,.guest-control-page .walk-in-field.full{grid-column:auto}}
  </style>
</head>
...
(() => {
  ...
  const render=(items)=>{
    ...
    tbody.innerHTML=rows.length?rows.map((row,index)=>{
      ...
      const toneClass=index%2===0?'log-record-even':'log-record-odd';
      ...
      return `<tr class="log-summary-row ${toneClass} status-${statusClass}"><td><div class="log-status"><span class="status-dot" aria-hidden="true"></span><span class="badge ${statusClass}">${esc(label(row.status))}</span></div></td><td><div class="cell-stack">${name?`<button type="button" class="guest-name-link" data-guest-detail="${index}">${esc(name)}</button>`:'—'}<small class="code">${identifier}</small></div></td><td>${latestOperation}</td><td><span class="attendance-state">${esc(attendanceStateLabel(row.attendance_state))}</span></td><td><div class="record-actions">${actions.join('')||'—'}</div></td></tr><tr class="log-detail-row ${toneClass}"><td colspan="5"><div class="log-detail-line"><span class="log-main-message">${esc(row.message||'—')}</span><span class="log-context">بازه: <strong>${esc(row.period_title||'—')}</strong></span><span class="log-context">بررسی: <strong class="code">${esc(row.attempted_at||'—')}</strong></span></div></td></tr>`;
