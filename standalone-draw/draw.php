...
const GUEST_STORE_PATH = __DIR__ . '/data/participants.json';
if (!defined('EVENTS_ROOT')) {
  define('EVENTS_ROOT', __DIR__ . '/runtime');
}
const STORE_PATH = __DIR__ . '/data/settings.json';
...
$drawEventCode = 'standalone';
$drawCaption = STANDALONE_DRAW_CAPTION;
...
if ($method === 'POST') {
  ...
  $submittedGuest = $payload['guest'];
  $submittedId = trim((string)($submittedGuest['id'] ?? ''));
  $submittedCode = preg_replace('/\D+/', '', (string)($submittedGuest['code'] ?? ''));
  $guest = null;
  foreach (loadStandaloneParticipants() as $candidate) {
    $candidateId = trim((string)($candidate['id'] ?? ''));
    $candidateCode = preg_replace('/\D+/', '', (string)($candidate['code'] ?? ''));
    if (
      ($submittedId !== '' && hash_equals($candidateId, $submittedId))
      || ($submittedId === '' && $submittedCode !== '' && hash_equals($candidateCode, $submittedCode))
    ) {
      $guest = $candidate;
      break;
    }
  ...
  if (!is_array($guest)) {
    echo json_encode(['status' => 'error', 'message' => 'Participant was not found in the current source.']);
    exit;
  }
...
$guestPool = loadStandaloneParticipants();
